...
/** Per-family drawable codepoints, generated by `npm run fonts:vendor`. */
export const COVERAGE_FILE = path.join(FONT_ASSET_DIR, 'coverage.json');
...
let coverageCache: Map<string, CoverageRange[]> | undefined;

/**
 * The codepoints the curated set can draw, keyed by family.
 *
 * Read from disk once. `loadFonts` deliberately does not populate this: overflow checks
 * and preview rendering want coverage without paying for ~830KB of font bytes.
 */
export async function loadCoverage(): Promise<Map<string, CoverageRange[]>> {
  if (coverageCache) return coverageCache;

  let parsed: Record<string, CoverageRange[]>;
  try {
    parsed = JSON.parse(await readFile(COVERAGE_FILE, 'utf8')) as Record<string, CoverageRange[]>;
  } catch {
    throw new RenderError(
      `Missing font coverage manifest at ${COVERAGE_FILE}. ` +
        'Run `npm run fonts:vendor --workspace=backend` to regenerate it.',
    );
  }

  coverageCache = new Map(Object.entries(parsed));
...
export function familyCovers(
  ...
  const ranges = coverage.get(family);
  return ranges ? covers(ranges, codepoint) : false;
}
